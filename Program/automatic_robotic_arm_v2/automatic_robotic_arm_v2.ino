...
void loop() 
{
  if (!digitalRead(pinBotaoGravar)) {  //Caso o botão gravar for pressionado, chama a função para gravar as posições
    ...
    //Serial.println("Tô no automatico");
    delay(250);
    modoAutomatico = true;
    automatico();
  }
...
void automatico() {
  ...
  while (modoAutomatico) 
  {
    for (x = 0; x < 64; x++) 
    {
      ...
      if (!digitalRead(pinBotaoReset)) { // Caso queira interromper o modo automático, basta apertar no botão reset
